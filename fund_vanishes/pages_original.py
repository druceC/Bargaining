...
# # Voting Page: Players vote on the selected proposal
# class VoterPage(Page):
#     form_model = 'group'  # ✅ Changed to group
#     form_fields = ['s1', 's2', 's3']  # ✅ Using specified form fields

#     def vars_for_template(self):
#         return {
#             'id': self.player.id_in_group,
#             'selected_proposal': self.group.selected_proposal
#         }

#     def before_next_page(self):
#         """Determine if the selected proposal is approved by majority vote."""
#         votes = [self.group.s1, self.group.s2, self.group.s3]
#         self.group.approved = votes.count(True) >= 2  # Majority vote required

#         if self.group.approved:
#             for p in self.group.get_players():
#                 p.earnings = self.group.selected_proposal
#         else:
#             for p in self.group.get_players():
#                 p.earnings = 0


# # Voting page for selected proposal
# class VoterPage(Page):
#     form_model = 'group'
#     form_fields = ['s1', 's2', 's3']  

#     def is_displayed(self):
#         """Only show this page to voters"""
#         return self.player.player_role == "Voter"  # ✅ Updated field name

#     def vars_for_template(self):
#         """Pass participant ID to the template"""
#         return {'id': self.player.id_in_group}

#     def before_next_page(self):
#         votes = [p.vote for p in self.group.get_players()]
#         self.group.approved = votes.count(True) >= 2  # Majority vote

#         if self.group.approved:
#             for p in self.group.get_players():
#                 p.earnings = p.proposal
#         else:
#             for p in self.group.get_players():
#                 p.earnings = 0
...
class ResultsPage(Page):
    ...
    def vars_for_template(self):
        ...
        #     for p in self.group.get_players():
        #         # p.earnings = self.group.selected_proposal
        #         player_id = f's{p.id_in_group}'  # Match participant ID to proposal
        #         p.earnings = cu(relevant_proposal[player_id])  # Convert value to Currency
        # # 0 earnings if proposal is not approved
        # else:
        #     for p in self.group.get_players():
        #         self.p.earnings = 0
    ...
    #         # Store the earnings from the randpomly selected round as the final payment
    #         self.participant.vars['final_payment'] = self.player.in_round(selected_round).earnings
...
class Part1d(Page):
    ...
    # form_fields = ['age', 'sex', 'gen', 'other_gender', 'gen_cgi', 'risk', 'party_like', 'econ']
...
class Part3b(Page):
    form_model = 'player'
    form_fields = ['occ', 'degree', 'volunt', 'volunt_hrs']
...
#     def before_next_page(self):
#         # Record response
#         # store_survey_response(self.player, "Part4a", self.form_fields)
#         # Increase survey step when the player moves to the next page
#         self.participant.vars['surveyStep'] += 1
...
# add to page sequance
# page_sequence = [SyncTop, Calculate, CalculateRounds, CalculateWaitpage, Proposer, VoterWaitPage, Voter,
#                  ProposerWaitPage, RoundDebrief, ProductionWaitPage, PeriodResult, SyncBottom,  # Survey,
#                  Part1a, Part1b, Part1c, Part1d, Part2a, Part2b, Part3a, Part3b,
#                  Part4a, Part4b, Part4c, Part5, Part6, Part7, Part8, Part9,
#                  FinalResults#, Fin
#                  ]
...
page_sequence = [
    ...
    # Part2b, # Part3a, 
    # Part3b, #Part4a, 
    # Part4c, Part6, Part7, Part8, Part9,
