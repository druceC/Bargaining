...
class SyncTop(WaitPage):
    ...
    def is_displayed(self):
        ...
        # For debugging print number of periods played
    ...
    def after_all_players_arrive(group, **kwargs):
        ...
        # Initialize list of players who reached reachedSyncTop if not already set
        if "ready_players" not in session.vars:
            session.vars["ready_players"] = []

        # Add new players to the ready list if not already there
        for p in subsession.get_players():
            if p.id_in_subsession not in session.vars["ready_players"]:
                session.vars["ready_players"].append(p.id_in_subsession)

        # For Debugging: Print how many players reached SyncTop
        print(f"[DEBUG] {len(session.vars['ready_players'])} players have reached SyncTop: {session.vars['ready_players']}")

        # GROUPING LOGIC ------------------------------------------------------------------------------------

        # Only run grouping logic (9 members) on round 1
        if subsession.round_number == 1:

            # If a least 9 players are ready and groups not yet set
            if len(session.vars["ready_players"]) >= 9 and "initial_group_matrix" not in session.vars:
                # Get all players in subsession
                all_players = subsession.get_players()
                
                # Select the first 9 from ready list
                ready_ids = session.vars["ready_players"][:9]
                players = [next(p for p in all_players if p.id_in_subsession == pid) for pid in ready_ids]

                # Shuffle and split into 3 groups of 3
                random.shuffle(players)
                group_matrix = [players[i:i + 3] for i in range(0, 9, 3)]

                # Identify the inactive players who are not in the active 9
                inactive_players = [p for p in all_players if p not in players]
                # Add inactive players as singleton groups (to satisfy oTree's group completeness rule)
                group_matrix += [[p] for p in inactive_players]

                # Set the full group matrix in oTree
                subsession.set_group_matrix(group_matrix)

                # Save only the active 3x3 structure for re-use in later rounds
                session.vars["initial_group_matrix"] = [[p.id_in_subsession for p in group] for group in group_matrix if len(group) == 3]

                # For debugging: Print group composition
                print("[DEBUG] Group matrix formed:")
                for i, group in enumerate(group_matrix):
                    print(f"  Subgroup {i + 1}: {[p.participant.label for p in group]}")

            # If group matrix already exists, reuse it for consistency
            elif "initial_group_matrix" in session.vars:
                # saved_matrix = list of IDs from earlier active players
                saved_matrix = session.vars["initial_group_matrix"]
                all_players = subsession.get_players()

                # Step 1: rebuild active players from saved matrix
                active_players = [
                    next(p for p in all_players if p.id_in_subsession == id_in_subsession)
                    for group in saved_matrix
                    for id_in_subsession in group
                ]

                # Step 2: group active players (based on saved structure)
                active_matrix = [
                    [next(p for p in all_players if p.id_in_subsession == id_in_subsession) for id_in_subsession in group]
                    for group in saved_matrix
                ]

                # Step 3: get inactive players (not in saved_matrix)
                inactive_players = [p for p in all_players if p not in active_players]

                # Step 4: put inactive players in their own singleton groups
                singleton_matrix = [[p] for p in inactive_players]

                # Step 5: combine both
                full_matrix = active_matrix + singleton_matrix

                # Set the full group matrix
                subsession.set_group_matrix(full_matrix)

        else:
            # For all rounds after round 1, reuse the saved group structure
            if "initial_group_matrix" in session.vars:
                saved_matrix = session.vars["initial_group_matrix"]
                all_players = subsession.get_players()
        ...
        # Loop through all players in current subsession
        for p in subsession.get_players():
            # Check if player has played fewer periods than max allowed
            if p.participant.vars.get('periods_played', 0) < Constants.no_periods:
                # Increment number of periods this player has completed
                p.participant.vars['periods_played'] += 1
                # Set a flag indicating the player should proceed to next period
